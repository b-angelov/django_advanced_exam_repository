...
class HolidaysRelatedManager(models.Manager):
    ...
    def get_easter_related(self,date: datetime.date, distance_obj=None,calendar=None):
        distance_dict = distance_obj['obj'](
            **distance_obj['data']
        ).get_distance()
    ...
    @set_calculus(CALENDAR)
    def get_christmas_related(self, date: datetime.date, distance_obj=None,calendar=None):
        distance_dict = distance_obj['obj'](
            **distance_obj['data']
        ).get_distance()
    ...
    def get_christmas_and_easter_related(self,date:datetime.date, calendar='JIG'):
        christmas_related = set(self.get_christmas_related(date, calendar=calendar).values_list(flat=True))
        easter_related = self.get_easter_related(date, calendar=calendar).values_list(flat=True)
